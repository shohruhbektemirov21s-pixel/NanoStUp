...
import { BuilderRegisterForm } from "@/features/auth/components/builder-register-form";
import { Link } from "@/i18n/navigation";

type P = { locale: string } | Promise<{ locale: string }>;

export async function generateMetadata({ params }: Readonly<{ params: P }>): Promise<Metadata> {
  const { locale } = await Promise.resolve(params);
  const t = await getTranslations({ locale, namespace: "BuilderSignup" });
  return { title: t("metaTitle"), description: t("metaDescription") };
}

export default async function BuilderSignupPage({ params }: Readonly<{ params: P }>) {
  const { locale } = await Promise.resolve(params);
  ...
  return (
    <div className="relative flex min-h-[78vh] flex-col items-center justify-center overflow-hidden px-4 py-16">
      <div
        ...
        aria-hidden
      />
      <Suspense fallback={<div className="h-[26rem] w-full max-w-md animate-pulse rounded-2xl bg-muted/50" />}>
        <BuilderRegisterForm />
      </Suspense>
