...
const TIER_ORDER: readonly TierId[] = ["basic", "pro", "premium"];
...
export function PricingView({ initialPlans }: Readonly<{ initialPlans: PublicPricingPlanRow[] }>) {
  ...
  const bySlug = useMemo(() => {
    const m = new Map<string, PublicPricingPlanRow>();
  ...
  const tierRows = useMemo(
    () =>
      TIER_ORDER.flatMap((id) => {
        const plan = bySlug.get(id);
        return plan ? [{ id, plan, highlight: id === "pro" as const }] : [];
      }),
    [bySlug],
  );
    ...
    try {
      ...
      if (!res.ok || !data.ok || !data.checkoutUrl) {
        ...
        if (code === "builder_auth_required") {
          ...
          setCheckoutError(t("payErrorPlanMissing"));
        } else {
  ...
  const nf = useMemo(() => new Intl.NumberFormat(intlLocaleForApp(locale), { maximumFractionDigits: 0 }), [locale]);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:py-20">
      ...
      <div className="mt-12 grid gap-6 lg:grid-cols-3">
        {tierRows.map(({ id: tierId, plan, highlight }, i) => {
          const price = totalUzsWholeFromManagedPlan(plan, months);
          const rawFeatures = t.raw(`tiers.${tierId}.features`);
          const featureList = Array.isArray(rawFeatures) ? (rawFeatures as string[]) : [];
          const extras: string[] = [];
          if (plan.generationLimit != null) {
            extras.push(t("limitGenShort", { count: plan.generationLimit }));
          }
          if (plan.exportLimit != null) {
            extras.push(t("limitExportShort", { count: plan.exportLimit }));
          }
          const lines = [...featureList, ...extras];
          return (
            <motion.article
              key={tierId}
              ...
              className={cn(
                ...
                highlight && "ring-2 ring-primary/40 lg:scale-[1.02]",
              )}
            >
              {highlight ? (
              ...
              <p className="mt-1 text-sm text-muted-foreground">{t(`tiers.${tierId}.tagline`)}</p>
              <p className="mt-5 text-3xl font-extrabold tracking-tight text-foreground">
                {nf.format(price)}
                <span className="text-base font-semibold text-muted-foreground">
                  {" "}
                  {t("pricingCurrencySuffix")} / {t("perPeriod", { months: String(months) })}
                </span>
              </p>
        ...
        >
          <div className="relative w-full max-w-md rounded-2xl border border-white/20 bg-background p-6 shadow-2xl dark:border-white/10">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {t("payModalBody", {
                tier: bySlug.get(modalTier)?.name ?? t(`tiers.${modalTier}.name`),
                months,
              })}
            </p>
